...
class SlipperyMazeEnv:
    ...
    def step(self, action):
        ...
        if cell == 'G':
            ...
            reward = -0.04  # small step penalty to encourage shorter paths
            done = False
...
def train_q_learning(
    ...
    epsilon_end=0.05,
    epsilon_decay=0.997,
    snapshot_points=None    # e.g. [100, 1000, 3000]
):
    ...
    # ### NEW: dictionary to store Q snapshots
    Q_snapshots = {}
    ...
    for episode in range(num_episodes):
        ...
        if (episode + 1) % episodes_window == 0:
            ...
            print(f"Episode {episode+1}/{num_episodes}, Success rate: {success_rate:.2f}")
            success_in_window = 0

        # ### NEW: capture snapshot if at this episode
        if (episode + 1) in snapshot_points:
            Q_snapshots[episode + 1] = Q.copy()

    # ### NEW: always save final Q-table
...
def main():
    ...
    env = SlipperyMazeEnv(slip_prob=0.2, max_steps=150)

    print("Training Q-learning agent on Slippery Maze...")
    Q, rewards, success_rates, Q_snapshots = train_q_learning(
        env,
        num_episodes=700,          # total training episodes
        alpha=0.1,
        gamma=0.99,
        epsilon_start=1.0,
        epsilon_end=0.05,
        epsilon_decay=0.997,
        snapshot_points=[100, 300, 500]   # NEW snapshots
    )
    # train_q_learning automatically stores snapshot at 700

    # --------- Plot learning curve (average return) ---------
    plt.figure()
    window = 50
    if len(rewards) >= window:
        ...
        plt.title("Q-learning on Slippery Maze")
    else:
        plt.plot(rewards)
        plt.xlabel("Episode")
        plt.ylabel("Return")
    ...
    # --------- Plot success rate (per 100-episode window) ---------
    plt.figure()
    plt.plot(success_rates)
    plt.xlabel("Hundreds of episodes")
    plt.ylabel("Success rate")
    plt.title("Goal-reaching success rate during training")
    plt.ylim(0, 1)
    plt.grid(True)
    plt.show()

    # --------- Show final learned greedy policy & values ---------
    print("\nFinal greedy policy and state values (after full training):")
    env.render_policy(Q)
    env.render_value(Q)

    # Evaluate final greedy policy
    evaluate_policy(env, Q, num_episodes=30)

    # --------- Animate early → mid → late → final policies ---------
    # Q_snapshots keys: 100, 300, 500, 700
    for point in sorted(Q_snapshots.keys()):
        print(f"\n=== Playing ONE attempt with Q after {point} training episodes ===")
        animate_with_pygame_one_episode(env, Q_snapshots[point], fps=5)
